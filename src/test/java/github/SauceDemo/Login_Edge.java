...
public class Login_Edge {
	...
	String evidenceFolder = "..\\SauceDemo\\evidenceFolder\\LogIn\\Edge\\";
	...
	public void login_Firefox_Credentials_Ok() throws IOException {
		...
		switch (faker.number().numberBetween(0, 2)) {
			...
			case 1: {
				edgeDriver.findElement(By.id("user-name")).sendKeys("problem_user");;
				break;
			}
			case 2: {
				edgeDriver.findElement(By.id("user-name")).sendKeys("performance_glitch_user");;
				break;
			}
		}
		
		//Assert: Username field is not empty
		Assert.assertNotNull(edgeDriver.findElement(By.id("user-name")).getText());
		
		//Password field
		edgeDriver.findElement(By.id("password")).sendKeys("secret_sauce");

		//Screenshot evidence
		screenshotEvidence = ((TakesScreenshot)edgeDriver).getScreenshotAs(OutputType.FILE);
		FileUtils.copyFile(screenshotEvidence, new File(evidenceFolder + "evidence_Login_Firefox_Credentials_Ok_1.png"));
		
		//Click on login button
		edgeDriver.findElement(By.id("login-button")).click();
		...
		Assert.assertNotEquals(edgeDriver.getCurrentUrl(), "https://www.saucedemo.com/");
	...
	public void login_Firefox_Credentials_Lockout() throws IOException {
		...
		edgeDriver.findElement(By.id("user-name")).sendKeys("locked_out_user");
		edgeDriver.findElement(By.id("password")).sendKeys("secret_sauce");
	...
	public void login_Firefox_Credentials_Username_Wrong() throws IOException {
		...
		//Random username
		edgeDriver.findElement(By.id("user-name")).sendKeys(faker.name().username());
		//Correct password
		edgeDriver.findElement(By.id("password")).sendKeys("secret_sauce");
	...
	public void login_Firefox_Credentials_Password_Wrong() throws IOException {
		...
		//Random password
		edgeDriver.findElement(By.id("password")).sendKeys(faker.internet().password());
